...
\begin{document}
	...
	\setheader{Math 240}{PAR 6}{Due: 11/17 (draft), 11/18 (final)}{}{Name:\underline{\hspace{2in}}}{}
	...
	Recall the setup from PAR 1 and 5. The newly-appointed queen of a newly-discovered land has commissioned 
	a new expedition to explore her territory.  This time, she sends two explorers: Emily and Aaron.  The explorers
	...
	\begin{enumerate}
		...
		is accurate.
		\item[Aaron] Aaron has miscalibrated \textit{all} of his equipment; he measures neither distances 
			nor directions accurately, but he can convert a location in his coordinates into Emily's 
			coordinates using the matrix $A = \mat{2 & 1 \\ 1 & 1}$.
	\end{enumerate}
	
	The queen also decrees that she will expand her territory by means of a linear transformation $T$; people 
	living at position vector $\vec{v}$ will have to move to $T(\vec{v})$. The realm's primary roads are Main 
	Street (along the line $y=2x$) and First Avenue (along the line $y=-2x$), intersecting at the royal palace. 
	The queen wishes to set up $T$ so that everyone living on Main Street will stay on Main Street but move 
	twice as far from the palace, and everyone living on First Avenue will stay on First Avenue but move three times as far from the palace.

	\vspace{1em}
	\begin{enumerate}

%		\item Compute the matrix $A^{-1}$.  What does $A^{-1}$ do? How can it be used?

		\item Draw a map showing the locations of Main Street, First Avenue, and the locations of 
			at least 3 villagers before the expansion.  Draw a second map showing Main Street, 
			First Avenue, and the same 3 villagers after the expansion.  For each villager in 
			each picture, label their coordinates both in \textbf{true} coordinates (i.e. Emily's), 
			and with what Aaron thinks their coordinates are.

%		\item From Emily's perspective, a point $(x,y)$ (in Emily's coordinates) is on Main Street if $y=2x$, 
%			and it is on First Avenue if $y=-2x$. Find analogous equations from Aaron's perspective. That 
%			is, find an equation that must be satisfied if $(a,b)$ (in Aaron's coordinates now) is on Main
%			Street, and a similar equation for First Avenue.

		\item The queen now relocates all of her citizens.  Emily wants to use a matrix to help find everyone's 
			new address.  Find a matrix $T_E$ such that if a citizen lives at position vector $\mat{x\\y}$ 
			(given in Emily's coordinates), then their new location is $T_E \mat{x\\y}$.

		\item Aaron wants to use a similar matrix to find people's new addresses, but in his coordinates.  
			Find a matrix $T_A$ such that if a citizen's position vector is $\mat{a\\b}$ (in Aaron's coordinates), 
			then their new location in Aaron's coordinates is $T_A \mat{a\\b}$.

		\item Explain, geometrically, why $A T_A A^{-1} = T_E$.
				
			(Geometrically, two matrices are called \emph{similar} if they represent the same transformation
			in different bases (\textit{bases} is the plural of basis).  Algebraically, the matrix $P$ is similar to the matrix $Q$ if
			there is an invertible matrix $X$ so $XPX^{-1}=Q$.)
