...
\begin{document}
	...
	\setheader{Math 240}{PAR 4}{Due: 10/27 (draft), 10/28 (final)}{}{Name:\underline{\hspace{2in}}}{}
	...
	\begin{enumerate}
		...
		\item Is there a linear transformation $\mathcal{T}$ such that $\mathcal{T}\left( \langle 1,1 \rangle \right) = \langle 1,2 \rangle$ and $\mathcal{T}\left( \langle 3,2 \rangle \right) = \langle 2,4 \rangle$?  Either give a matrix for such a transformation, or explain why there isn't one.
		
		\item Is there a linear transformation $\mathcal{R}$ such that $\mathcal{R}\left( \langle 1,2 \rangle \right) = \langle 1,1 \rangle$, and $\mathcal{R}\left( \langle 2,4 \rangle \right) = \langle 3,2 \rangle$?  Either give a matrix for such a transformation, or explain why there isn't one.
		
		\item If you answered ``yes'' to either of the above questions, are there other linear transformations that do the same thing?  Give an example, or explain why there aren't.
		
	\end{enumerate}
	
	Suppose $\{v_1, v_2, v_3\}$ is a linearly independent set, and $\{w_1, w_2, w_3\}$ is a linearly dependent set in $\R^3$.
