...
\begin{document}
	...
	\begin{enumerate}
		\item
		Consider the stretched and skewed coordinate system $\mathcal {SC}$ for $\R^2$ where
		the point $(a,b)$ in $\mathcal {SC}$ coordinates is related to $(x,y)$ in rectangular
		coordinates by
		\[
			x=a-b\qquad \qquad y=2a+b.
		\]
		Let $B=\{(x,y): 0\leq x\leq 1\text{ and }0\leq y\leq 1\}$ specified in rectangular coordinates.
		\begin{enumerate}
			\item Specify $B$ in $\mathcal {SC}$ coordinates.

			\item Compute the volume form for $\mathcal {SC}$ coordinates.

			\item Set up an integral to find the area of $B$ using $\mathcal {SC}$ coordinates. (Hint,
				you already know the area is 1, so make sure to check your answer!)
		\end{enumerate}
